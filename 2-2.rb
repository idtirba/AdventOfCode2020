...
9-13 n: nglxnhnnrktnnfznn".split(/\n+/)
...
input.each do |s|
  a = s.split(' ')
  range = a.first.split('-')
  first = range.first.to_i-1
  last = range.last.to_i-1
  char = a[1][0]
  string = a.last
  validPasswords+=1 if (string[first] == char || string[last] == char) && string[first] != string[last]
end
