...
messages="abbaabbbababaaaabbababbbbbbaabbaaabaabbb
...
aaaabbabbbbbabbbbaabbbaabbaababbaaabaaabbbaaabab".split(/\n/)
...
def getRegexFor(key, hash)
  return @cache[key] if @cache[key]
  ex = ""
  return key unless hash[key]
  hash[key].each_with_index do |v, i|
    v.split(' ').each do |n|
      ex += getRegexFor(n, hash)
    end
    ex += '|' if hash[key][i+1]
  end
  ex = '(' + ex + ')' if ex.include?('|')
...
regex = Regexp.new("^#{getRegexFor('0', hash)}$")

p messages.select {|m| m =~ regex}.count

# part 2 had to lookup what people did
# since zero rule is 0: 8 11 and
# 8: 42 | 42 8
# 11: 42 31 | 42 11 31
# 8 is 42 repeating and 11 is optionally sandiwched 11 between 42 and 31
# find 42 and 31 to build regex with these rules

# get original rule
regex42 = getRegexFor('42', hash)
regex31 = getRegexFor('31', hash)

regex8 = "((#{regex42})+)"
# recursive nested regex right here
# ?<rec> names/self references expression as rec
# \\g<rec>? where \\g recurses the subpattern named rec as many times (?)
regex11 = "(?<rec>#{regex42}\\g<rec>?#{regex31})"

# rule 0 is 8 11
regex0 = Regexp.new("^#{regex8}#{regex11}$")

p messages.select {|m| m =~ regex0}.count