input = "plaid beige bags contain 3 drab magenta bags.
...
striped coral bags contain 4 wavy gray bags, 5 faded purple bags.".split(/\n/)
...
input.each do |line|
  valueMap[line[0..line.index("bags")-2]] = 0
end

input.each do |line|
  if line.include?('no other bags')
    color = line[0..line.index("bags")-2]
    valueMap[color] = 1
    colorsAssigned += 1
  end
end

valueMap.keys.each do |color|
  input.each do |line|
    line.gsub!(color, valueMap[color].to_s + " ") if valueMap[color] != 0
  end
end

while colorsAssigned != valueMap.keys.length do
  input.each do |line|
    currentColor = line[0..line.index("bags")-2]
    colorsContained = 0
    valueMap.keys.each do |key|
      colorsContained += 1 if line.include?(key)
    end
    if colorsContained == 1 && valueMap[currentColor] == 0
      value = 0
      array = line.split(' ')
      valuesAdded = 0
      array.each_with_index do |n, index|
        value += array[index].to_i * array[index+1].to_i if isNumber?(array[index]) && isNumber?(array[index+1])
      end
      valueMap[currentColor] = value + 1
      colorsAssigned+=1
      input.each do |l|
        l.gsub!(currentColor, valueMap[currentColor].to_s + " ") if valueMap[currentColor] != 0
      end
    end
  end
end

puts valueMap["shiny gold"] - 1