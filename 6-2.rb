...
answerCount = Hash[answers.collect { |letter| [letter, 0] } ]
...
input.each_with_index do |n, index|
  currentLetters+= n
  pCount+=1 if n.length != 0
  if n.length == 0 || index == input.length-1
    answers.each do |l|
      answerCount[l] = answerCount[l] + 1 if currentLetters.scan(/(?=#{l})/).count == pCount
    end
    currentLetters = ""
    pCount = 0
  end
...
answerCount.values.inject(0) {|sum, i|  sum + i }