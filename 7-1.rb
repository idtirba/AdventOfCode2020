...
def colorsThatCanContainColor(color, input)
  colors = []
  input.each do |line|
    colors.push(line[0..line.index("bags")-2]) if line.include?(color) && line.index(color) != 0
  end
  return colors
end
...
currentColors = colorsThatCanContainColor("shiny gold", input)
colors+=currentColors

while currentColors.length > 0 do
  nextColors = []
  currentColors.each do |color|
    nextColors += colorsThatCanContainColor(color, input)
  end
  colors+=nextColors
