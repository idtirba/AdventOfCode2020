...
input.each_with_index { |n, index| input[index] = input[index].split('')}

def getTopLeft(i, x, y, rowLen, colLen)
  cX = x-1
  cY = y+1
  while cX >= 0 && cY < rowLen do
    return i[cY][cX] if i[cY][cX] != '.'
    cX-=1
    cY+=1
  end
  return '.'
...
def getTop(i, x, y, rowLen, colLen)
  cY = y+1
  while cY < rowLen do
    return i[cY][x] if i[cY][x] != '.'
    cY+=1
  end
  return '.'
end

def getTopRight(i, x, y, rowLen, colLen)
  cX = x+1
  cY = y+1
  while cX >= 0 && cY < rowLen do
    return i[cY][cX] if i[cY][cX] != '.'
...
def getLeft(i, x, y, rowLen, colLen)
  cX = x-1
  while cX >= 0 do
    return i[y][cX] if i[y][cX] != '.'
    cX-=1
  end
  return '.'
end

def getRight(i, x, y, rowLen, colLen)
  cX = x+1
  while cX < colLen do
    return i[y][cX] if i[y][cX] != '.'
    cX+=1
...
def getBottomLeft(i, x, y, rowLen, colLen)
  cX = x-1
  cY = y-1
  while cX >= 0 && cY >= 0 do
    return i[cY][cX] if i[cY][cX] != '.'
...
def getBottom(i, x, y, rowLen, colLen)
  cY = y-1
  while cY >= 0 do
    return i[cY][x] if i[cY][x] != '.'
    cY-=1
...
def getBottomRight(i, x, y, rowLen, colLen)
  cX = x+1
  cY = y-1
  while cX < colLen && cY >= 0 do
    return i[cY][cX] if i[cY][cX] != '.'
    cX+=1
    cY-=1
  end
...
def matchingAdjacentSeats(i, y, x, val, rowLen, colLen)
  count = 0
  adjacent = []
  adjacent.push(getTopLeft(i, x, y, rowLen, colLen))
  adjacent.push(getTop(i, x, y, rowLen, colLen))
  adjacent.push(getTopRight(i, x, y, rowLen, colLen))
  adjacent.push(getLeft(i, x, y, rowLen, colLen))
  adjacent.push(getRight(i, x, y, rowLen, colLen))
  adjacent.push(getBottomLeft(i, x, y, rowLen, colLen))
  adjacent.push(getBottom(i, x, y, rowLen, colLen))
  adjacent.push(getBottomRight(i, x, y, rowLen, colLen))
  adjacent.each do |n|
    if val == 'L'
      count+=1 if n == '#'
    else
...
while true do
  changedSeats = 0
  nextGrid = Marshal.load(Marshal.dump(input))
  input.each_with_index do |val, row|
    val.each_with_index do |val, col|
      if input[row][col] == '#'
        if matchingAdjacentSeats(input, row, col, '#', rowLen, colLen) > 4
          nextGrid[row][col] = 'L'
          changedSeats+=1
        end
      elsif input[row][col] == 'L'
        if matchingAdjacentSeats(input, row, col, 'L', rowLen, colLen) == 0
          nextGrid[row][col] = '#'
          changedSeats+=1
        end
      end
