...
def newCalculate(ex)
  while ex.include?('+') do
    x = (0 ... ex.length).find_all { |x| ex[x,1] == '+' }.sort!.first
    t1,t2 = x,x
    while ex[t1-1] =~ /[[:digit:]]/ && t1 > 0 do t1-=1 end
    while ex[t2+1] =~ /[[:digit:]]/ && t2 < ex.length-1 do t2+=1 end
    val = eval(ex[t1..t2])
    start = t1-1<0 ? "" : ex[0..t1-1]
    ending = t2+1>=ex.length ? "" : ex[t2+1..ex.length-1]
    ex = start + val.to_s + ending
  end
  return eval(ex)
end
...
def simplify(i)
  ...
  val = newCalculate(i[(openParen+1)..(closParen-1)])
