...
def matchingAdjacentSeats(i, y, x, val, rowLen, colLen)
  count = 0
  adjacent = []
  adjacent.push(i[y+1][x-1]) if y+1 < rowLen && x-1 >= 0
  adjacent.push(i[y+1][x]) if y+1 < rowLen
  adjacent.push(i[y+1][x+1]) if y+1 < rowLen && x+1 < colLen

  adjacent.push(i[y-1][x-1]) if y-1 >= 0 && x-1 >= 0
  adjacent.push(i[y-1][x]) if y-1 >= 0
  adjacent.push(i[y-1][x+1]) if y-1 >= 0 && x+1 < colLen

  adjacent.push(i[y][x+1]) if x+1 < colLen
  adjacent.push(i[y][x-1]) if x-1 >= 0
  adjacent.each do |n|
    if val == 'L'
      count+=1 if n == '#'
    else
...
while true do
  ...
  input.each_with_index do |val, row|
    val.each_with_index do |val, col|
      if input[row][col] == '#'
        if matchingAdjacentSeats(input, row, col, '#', rowLen, colLen) > 3
          nextGrid[row][col] = 'L'
          changedSeats+=1
        end
